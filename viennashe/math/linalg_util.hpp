...
namespace viennashe
{
  namespace math
  {
    ...
    template<typename VectorType>
      VectorType
      subtract (VectorType const &x, VectorType const &y)
      {
	assert(
	    x.size () == y.size ()
		&& bool ("Size mismatch for subtracting y from x"));
	...
	sparse_matrix (size_type num_rows, size_type num_cols) :
	    data_ (num_rows), zero_ (0)
	{
	  assert(
	      num_cols == num_rows && bool ("Only square matrices supported."));
	  (void) num_cols;
	}
	...
	{
	  assert(
	      i < data_.size () && j < data_.size ()
		  && bool ("Access to matrix out of range!"));
	  return data_.at (i)[j];
	}
	...
	trans () const
	{
	  ...
	  for (size_type i = 0; i < data_.size (); ++i)
	    {
	      for (const_iterator2 it = data_[i].begin ();
		  it != data_[i].end (); ++it)
	...
	self_type
	operator* (NumericT factor) const
	{
	...
	self_type&
	operator+= (self_type B)
	{
	  for (size_type i = 0; i < B.size1 (); ++i)
	    {
	      row_type row_i = B.row (i);
	      for (const_iterator2 it = row_i.begin (); it != row_i.end ();
		  ++it)
    ...
    template<typename NumericT, typename VectorT>
      VectorT
      row_normalize_system (sparse_matrix<NumericT> &system_matrix,
			    VectorT &rhs)
      {
	...
	/*
	 * Count types of Double types
	 * */
	//check_types (system_matrix, rhs);
	VectorT scale_factors (rhs.size (), 1.0);

	// columm-scaling
	sparse_matrix<NumericT> system_matrixc = system_matrix.trans ();
	for (std::size_t i = 0; i < system_matrixc.size1 (); ++i)
	  {
	    RowType &col_i = system_matrixc.row (i);

	    //         log-scale
	    double row_norm = 0.0;
	    //      double size = col_i.size();
	    for (AlongRowIterator iter = col_i.begin (); iter != col_i.end ();
		++iter)
	      {
		double temp = std::log (std::fabs (iter->second));
		row_norm += temp;
	      }
	    row_norm = std::exp (row_norm / col_i.size ());
	    //    normalize such that diagonal entry becomes positive:
	    if (system_matrixc (i, i) < 0.0)
	      row_norm *= -1.0;

	    // scale col accordingly:
	    for (AlongRowIterator iter = col_i.begin (); iter != col_i.end ();
		++iter)
	      {
	    ...
	    scale_factors[i] /= row_norm;
	  }
	system_matrix = system_matrixc.trans ();
	...
	for (std::size_t i = 0; i < system_matrix.size1 (); ++i)
	  {
	    ...
	    double row_norm = 0.0;
	    for (AlongRowIterator iter = row_i.begin (); iter != row_i.end ();
		++iter)
	      {
...
//	check_types (system_matrix, rhs);
	...
	for (std::size_t i = 0; i < system_matrix.size1 (); ++i)
	  {
		...
		iter != system_matrix.row (i).end (); ++iter)
	      {
    ...
    /*
     * Check types of Numeric to find the value denormal and others
     * */
    template<typename NumericT, typename VectorType>
      void
      check_types (sparse_matrix<NumericT> const &system_matrix,
		   VectorType const &x)
      {
	typedef typename sparse_matrix<NumericT>::const_iterator2 Iterator2;
	typedef typename sparse_matrix<NumericT>::row_type RowType;
	double minimaldenormal, minimalnormal;
	int Total, Inf, NaN, subnor, nor, zero;
	int Total2, Inf2, NaN2, subnor2, nor2, zero2;
	Total = Inf = NaN = subnor = nor = zero = 0;
	Total2 = Inf2 = NaN2 = subnor2 = nor2 = zero2 = 0;
	minimaldenormal = minimalnormal = 1.0;
	for (std::size_t i = 0; i < system_matrix.size1 (); ++i)
	  {
	    RowType const &row_i = system_matrix.row (i);
	    for (Iterator2 iter2 = row_i.begin (); iter2 != row_i.end ();
		++iter2)
	      {
		switch (std::fpclassify (iter2->second))
		  {
		  case FP_INFINITE:
		    Inf++, Total++;
		    break;
		  case FP_NAN:
		    ...
		    break;
		  case FP_SUBNORMAL:
		    subnor++, Total++, minimaldenormal =
			iter2->second > minimaldenormal ?
			    minimaldenormal : iter2->second;
		    break;
		  case FP_NORMAL:
		    nor++, Total++, minimalnormal =
			iter2->second > minimalnormal ?
			    minimalnormal : iter2->second;
		    break;
		  case FP_ZERO:
		    zero++, Total++;
		    break;
	...
	std::cout << "Matrix: Total:" << Total << " Subnormal " << subnor
	    << " normal " << nor << " Zero " << zero << " Inf " << Inf
	    << " Nan " << NaN << std::endl;
	std::cout << "Vector: Total:" << Total2 << " Subnormal " << subnor2
	    << " normal " << nor2 << " Zero " << zero2 << " Inf " << Inf2
	    << " Nan " << NaN2 << std::endl;
	printf ("minimaldenormal: %f %A | minimalnormal: %f %A ",
		minimaldenormal, minimaldenormal, minimalnormal, minimalnormal);
	//std::cout << "minimaldenormal :" << minimaldenormal <<" " << std::bitset<64>(minimaldenormal)   << " minimalnormal :" << minimaldenormal << std::bitset<64>(minimalnormal) <<  std::endl;
      }
    ...
    template<typename NumericT, typename VectorType>
      VectorType
      prod (sparse_matrix<NumericT> const &system_matrix, VectorType const &x)
      {
	typedef typename sparse_matrix<NumericT>::const_iterator2 Iterator2;
	typedef typename sparse_matrix<NumericT>::row_type RowType;
	...
	dense_matrix (size_type rows, size_type cols) :
	    data_ (rows * cols + 1), size1_ (rows), size2_ (cols)
	{
	  assert(
	      rows == cols && bool ("Only square dense matrices supported!"));
	}
