...
namespace viennashe
{
  namespace solvers
  {
    ...
    {
        enum { dense_linear_solver,         /// Gauss solver (use for systems up to ~1000-by-1000 only)
    ...
    class linear_solver_config : public linear_solver_ids
    {
      public:
        ...
        linear_solver_config() :
         id_(linear_solver_ids::serial_linear_solver),
         tol_(1e-13),
         max_iters_(1000),
         ilut_entries_(60),
         ilut_drop_tol_(1e-4),
        ...
        void tolerance(double tol) { if (tol > 0) tol_ = tol; }

        /** @brief Returns the maximum number of iterative linear solver iterations */
        std::size_t max_iters() const { return max_iters_; }
        /** @brief Sets the maximum number of iterations within the iterative linear solver */
        void max_iters(std::size_t iters) { max_iters_ = iters; }

        /** @brief Returns the maximum number of entries in each of the two factors of the ILUT preconditioner */
        std::size_t ilut_entries() const { return ilut_entries_; }
        /** @brief Sets the maximum number of entries in each of the two factors of the ILUT preconditioner */
        void ilut_entries(std::size_t num) { ilut_entries_ = num; }

        /** @brief Returns the drop tolerance for the ILUT preconditioenr */
        double ilut_drop_tolerance() const { return ilut_drop_tol_; }
        /** @brief Sets the drop tolerance for the ILUT preconditioner. Non-positive values are ignored */
        void ilut_drop_tolerance(double tol) { if (tol > 0) ilut_drop_tol_ = tol; }

        block_preconditioner_boundaries_container       & block_preconditioner_boundaries()       { return block_precond_boundaries_; }
        block_preconditioner_boundaries_container const & block_preconditioner_boundaries() const { return block_precond_boundaries_; }
    ...
    {
        enum { gummel_nonlinear_solver,      /// Gummel iteration
    ...
    class nonlinear_solver_config : public nonlinear_solver_ids
    {
      public:
        ...
        void set(std::string solver_name)
        {
          ...
          if (solver_name == "gummel_nonlinear_solver" || solver_name == "gummel")
          {
            id_ = nonlinear_solver_ids::gummel_nonlinear_solver;
          }
          else if (solver_name == "newton_nonlinear_solver" || solver_name == "newton")
          {
        ...
        void damping(double d) { if (damping_ >= 0) damping_ = d; }
