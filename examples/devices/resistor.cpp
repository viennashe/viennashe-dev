...
void init_device(DeviceType & device, double vcc)
{
  ...
  device.set_doping_n(1e25);
  device.set_doping_p(1e7);
...
int main(int argc, char **argv)
{
   ...
   **/
  generator_params.add_segment(0.0, 1e-7, 11); //start at x=0, length 1um, 11 points // max 25
  ...
  // Linear Solver
  config.linear_solver().max_iters(1);
  ...
  edfwriter(device, viennashe::util::any_filter(),
      she_simulator.edf(viennashe::ELECTRON_TYPE_ID), "resistor_edf.dat");
  ...
  viennashe::she::current_density_wrapper<DeviceType, she_quan_type> Jn(device,
      config, she_simulator.quantities().electron_distribution_function());

  viennashe::io::write_cell_quantity_for_gnuplot(Jn, device, "resistor_Jn.dat");
  viennashe::io::write_cell_quantity_for_gnuplot(
      she_simulator.electron_density(), device, "resistor_n.dat");

  /** Write all computed quantities to VTK for inspection with e.g. ParaView: **/
  viennashe::io::write_quantities_to_VTK_file(she_simulator,
      "resistor_she_quan");
   ...
   **/
  std::cout << "* main(): Computing SHE (electrons only) with EE scattering ..."
      << std::endl;
  config.nonlinear_solver().max_iters(1);
  ...
  she_simulator_ee.set_initial_guess(viennashe::quantity::potential(),
      she_simulator.potential());
  she_simulator_ee.set_initial_guess(viennashe::quantity::electron_density(),
      she_simulator.electron_density());
  she_simulator_ee.set_initial_guess(viennashe::quantity::hole_density(),
      she_simulator.hole_density());
  ...
  edfwriter(device, viennashe::util::any_filter(),
      she_simulator_ee.edf(viennashe::ELECTRON_TYPE_ID), "resistor_edf_ee.dat");

  /** Let's also write the electron density and the average carrier energy to a plain text file:
   **/
  viennashe::io::write_cell_quantity_for_gnuplot(
      she_simulator_ee.electron_density(), device, "resistor_n_ee.dat");

  viennashe::she::carrier_energy_wrapper<she_quan_type> avge(config,
      she_simulator_ee.quantities().electron_distribution_function());
  viennashe::io::write_cell_quantity_for_gnuplot(avge, device,
      "resistor_avg_ee.dat");
