...
int main()
{
  ...
  typedef viennagrid::result_of::point<SegmentType>::type       PointType;

  typedef viennagrid::result_of::const_cell_range<viennagrid::triangular_2d_mesh>::type  CellContainer;
  ...
  **/
  ...
  config.set_hole_equation(viennashe::EQUATION_SHE); // SHE for holes
  ...
  config.nonlinear_solver().damping(0.04);
...
//      << std::endl;
  ...
  /*
  * POSTPROCESS THE EDFs to asses the HCD damage
  */

  //SegmentType const & gate_oxide     = device.segment(2);
  //SegmentType const & body           = device.segment(6);

  PointType dipole(0,0,0); // no dipole interaction

  viennashe::models::hcd_parameters hcdparams;
